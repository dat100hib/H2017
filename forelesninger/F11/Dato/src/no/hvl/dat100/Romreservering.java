...
public class Romreservering {
	...
	/**
	 * Skjuler konstruktorer slik at ingen andre kan lage instanser av denne klassen.
	 */
	...
	public static void main(String[] args) {
		...
		do {
			...
			if (dato.lovlig() && startTid.lovlig() && sluttTid.lovlig()) {
				...
				Reservasjon reservasjon = new Reservasjon(dato, startTid, sluttTid, ref);

				boolean ok = true;
				int nr = 0;
				//Test på kollisjon med andre reservajoner
				while (ok && nr < antall) {
					if (reservasjoner[nr].kollisjonMed(reservasjon)) {
						ok = false;
					} else {
						nr++;
					}
		...
		for (int i = 0; i < antall; i++) {
			reservasjonsVisning += reservasjonsListe[i] + "\n";
		}
